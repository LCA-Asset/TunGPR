...
def forward_convert1(coordinate):
    """
    :param coordinate: format [y_c, x_c, h, w, theta]
    :return: format [y1, x1, y2, x2, y3, x3, y4, x4]
    """
    boxes = []
    for rect in coordinate:
        box = cv2.boxPoints(((rect[0], rect[1]), (rect[2], rect[3]), rect[4]*57.3-180))
        box = np.reshape(box, [-1, ])
        boxes.append([box[0], box[1], box[2], box[3], box[4], box[5], box[6], box[7], rect[5]])

    return np.array(boxes, dtype=np.float32)

def forward_convert2(coordinate):
    boxes=[]
    for rect in coordinate:
        boxes.append([rect[0], rect[1], rect[2], rect[1], rect[0], rect[3], rect[2], rect[3], rect[4]])
        #通过tensorboard的记录发现，这里x和y应该转置，但具体为什么，我不清楚
    return np.array(boxes, dtype=np.float32)
