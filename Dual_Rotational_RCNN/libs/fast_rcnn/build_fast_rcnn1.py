...
class FastRCNN(object):
    def __init__(self,
                 img_batch,feature_pyramid, rpn_proposals_boxes, rpn_proposals_scores,
                 img_shape,
                 roi_size,
                 scale_factors,
        ...
        self.fast_rcnn_all_level_rois, self.fast_rcnn_all_level_proposals = self.get_rois(self.roi_size, self.roi_size)
        self.fast_rcnn_all_level_rois1, _ = self.get_rois(22, 6)
        self.fast_rcnn_all_level_rois2, _ = self.get_rois(6, 22)
    ...
    def get_rois(self, roi_w, roi_h):
       ...
       # levels = self.assign_level()
        ...
        with tf.variable_scope('fast_rcnn_roi'):
            ...
            #all_level_proposal_list.append(self.rpn_proposals_boxes)
            ...
            level_i_cropped_rois = tf.image.crop_and_resize(self.feature_pyramid,
                                                                boxes=tf.transpose(tf.stack([normalize_ymin, normalize_xmin,
                                                                                             normalize_ymax, normalize_xmax])),
                                                                box_ind=tf.zeros(shape=[tf.shape(self.rpn_proposals_boxes)[0], ],
                                                                                 dtype=tf.int32),
                                                                crop_size=[roi_w, roi_h]
                                                                )
                #level_i_rois = crop(self.feature_pyramid['P%d' % i],
                                                        #boxes=tf.transpose(tf.stack([ xmin, ymin, 
                                                                                      #xmax, ymax ])),
                                                        #batch_inds=tf.zeros(shape=[tf.shape(level_i_proposals)[0], ],
                                                                                 #dtype=tf.int32),
            ...
            level_i_rois = slim.max_pool2d(level_i_cropped_rois,
                ...
                #all_level_roi_list.append(level_i_cropped_rois)

            return level_i_rois, self.rpn_proposals_boxes
    ...
    def fast_rcnn_net(self):
        ...
        with tf.variable_scope('fast_rcnn_net_rotate'):
            with slim.arg_scope([slim.fully_connected], weights_regularizer=slim.l2_regularizer(self.weight_decay)):
                ...
                flatten_rois_features1_rotate = slim.flatten(self.fast_rcnn_all_level_rois1)
                flatten_rois_features2_rotate = slim.flatten(self.fast_rcnn_all_level_rois2)
                flatten_rois_features_rotate = tf.concat([flatten_rois_features_rotate, flatten_rois_features1_rotate,
                                                       flatten_rois_features2_rotate], axis=1)
    ...
    def fast_rcnn_proposals_rotate(self, decode_boxes, scores):
        ...
        with tf.variable_scope('fast_rcnn_proposals'):
            ...
            decode_boxes1 = tf.reshape(decode_boxes, [-1,  5])
            decode_boxes_list = tf.unstack(decode_boxes, axis=1)
            score_list = tf.unstack(scores[:, 1:], axis=1)
            after_nms_boxes = []
            after_nms_scores = []
            category_list = []
            predict_img = draw_box_with_color_rotate(self.img_batch,
                                                        decode_boxes1,
                                                        text=tf.shape(decode_boxes1)[0])
            tf.summary.image('nmszhiqianfast', predict_img)
            for per_class_decode_boxes, per_class_scores in zip(decode_boxes_list, score_list):
            ...
            # all_nms_boxes = boxes_utils.clip_boxes_to_img_boundaries(all_nms_boxes,
            #                                                          img_shape=self.img_shape)
            top_detections_in_img = draw_box_with_color_rotate(self.img_batch,
                                                        all_nms_boxes,
                                                        text=tf.shape(all_nms_boxes)[0])
            tf.summary.image('nmsfast', top_detections_in_img)
