...
class RPN(object):
    ...
    def get_feature_maps(self):
        ...
        with tf.variable_scope('get_feature_maps'):
            if self.net_name == 'resnet_v1_50':
                ...
                feature_maps_dict = {
                    #'C1': self.share_net['vgg_16/pool1'],  # [56, 56]
                    #'C2': self.share_net['vgg_16/pool2'],  # [56, 56]
                    #'C3': self.share_net['vgg_16/pool3'],  # [28, 28]
                    #'C4': self.share_net['vgg_16/pool4'],  # [14, 14]
                    #'C5': self.share_net['vgg_16/pool5']  # [7, 7]
                    'C2': self.share_net['vgg_16/conv3_3'],# [56, 56]
                    'C3': self.share_net['vgg_16/conv4_4'],  # [28, 28]
                    'C4': self.share_net['vgg_16/conv5_4'],  # [14, 14]
                    'C5': self.share_net['vgg_16/conv5_2'],  # [28, 28]
                    'C6': self.share_net['vgg_16/conv4_2'],  # [14, 14]
                    #'C5': self.share_net['vgg_16/pool5']  # [7, 7]
                }
            else:
    ...
    def rpn_losses(self):
        with tf.variable_scope('rpn_losses'):
            ...
            minibatch_anchors = tf.gather(self.anchors, minibatch_indices) #根据minibatch_indices对anchors进行切片
            minibatch_encode_boxes = tf.gather(self.rpn_encode_boxes, #根据minibatch_indices对rpn_encode_boxes进行切片
                                               minibatch_indices)  # 即网络预测出的每个minibatch中每个anchor的回归值，（256，4）
            ...
            # 因为这里的gtboxes是原始坐标，所以需要encode成anchors的调整参数，所有我后面需要的信息，都在encode_and_decode.py这个文件里
            ...
            minibatch_decode_boxes = encode_and_decode.decode_boxes(encode_boxes=minibatch_encode_boxes,
                                                                    # 这个是计算GIOU loss需要的参数之一，pred boxes
                                                                    reference_boxes=minibatch_anchors,
                                                                    scale_factors=self.scale_factors)
            ...
            with tf.variable_scope('rpn_location_loss'):
                ...
                if cfgs.LOSS == 'iou':
                    location_loss = losses.giou_losses(predict_boxes=minibatch_decode_boxes,
                                                       gtboxes=minibatch_anchor_matched_gtboxes,
                                                       g=cfgs.IS_GIOU)
                #                location_loss = losses.diou_losses(predict_boxes=minibatch_decode_boxes,
                #                                                    gtboxes=minibatch_anchor_matched_gtboxes)

                slim.losses.add_loss(2 * location_loss)  # add smooth l1 loss to losses collection
            ...
            # return minibatch_indices, minibatch_anchor_matched_gtboxes, object_mask, minibatch_labels_one_hot, self.anchors, self.rpn_scores, \
                   # location_loss, classification_loss, minibatch_encode_boxes, minibatch_encode_gtboxes, self.rpn_encode_boxes
    ...
    def rpn_proposals(self):
        with tf.variable_scope('rpn_proposals'):
            ...
            return rpn_proposals_boxes, rpn_proposals_scores, rpn_decode_boxes, rpn_decode_boxes, valid_indices, valid_boxes, valid_scores
