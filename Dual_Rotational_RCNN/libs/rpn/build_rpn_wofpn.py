...
class RPN(object):
    ...
    def make_anchors(self):
        with tf.variable_scope('make_anchors'):
            featuremap_height, featuremap_width = tf.shape(self.share_net)[1], tf.shape(self.share_net)[2]
            ...
            tmp_anchors = make_anchor.make_anchors(self.base_anchor_size_list, self.anchor_scales, self.anchor_ratios,
                                                           featuremap_height,  featuremap_width, self.stride)
                                                           #name='make_anchors')
        ...
        with tf.variable_scope('rpn_net'):
            with slim.arg_scope([slim.conv2d], weights_regularizer=slim.l2_regularizer(self.rpn_weight_decay)):
                if self.share_head:
                   ...
                   reuse_flag = None
                   scope_list = ['conv2d_3x3' ,'rpn_classifier', 'rpn_regressor']
                rpn_conv2d_3x3 = slim.conv2d(inputs=self.share_net,
                                                 num_outputs=256,
                                                 kernel_size=[3, 3],
                                                 stride=1,
                                                 scope=scope_list[0],
            ...
            return rpn_encode_boxes, rpn_box_scores#yucekuang
    ...
    def rpn_proposals(self):
        with tf.variable_scope('rpn_proposals'):
            rpn_decode_boxes = encode_and_decode.decode_boxes(encode_boxes=self.rpn_encode_boxes, #yucepianyiliang
