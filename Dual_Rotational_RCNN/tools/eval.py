...
def eval_ship(img_num):
    with tf.Graph().as_default():
        ...
        rpn_proposals_boxes, rpn_proposals_scores, rpn_decode_boxes, rpn_decode_boxes, valid_indices, valid_boxes, valid_scores\
        ...
        with tf.Session(config=config) as sess:
            ...
            for i in range(img_num):
                ...
                _img = help_utils.draw_box_cv(str(_img_name_batch[0])[2:].split('.jpg')[0]+'.jpg', _fast_rcnn_decode_boxes,
                                              _detection_category, _fast_rcnn_score, is_valid=True)

                #os.makedirs(cfgs.TEST_SAVE_PATH + cfgs.VERSION)
                # cv2.imwrite(cfgs.TEST_SAVE_PATH + '/{}_horizontal_fpn.jpg'.format(str(_img_name_batch[0])), _img_batch_fpn_horizonal)
                cv2.imwrite(cfgs.TEST_SAVE_PATH + cfgs.VERSION + '/{}.jpg'.format(str(_img_name_batch[0]).split('.jpg')[0]),
                            _img)
...
def get_single_label_dict(predict_dict, gtboxes_dict, label):
    '''输入预测结果框的集和，和GT结果框的集和，按照对应的label挑选出结果框，重新放入rboxes和gboxes中'''
    rboxes = {}
    gboxes = {}
    ...
    for i in range(len(rbox_images)):
        ...
        for pre_box in predict_dict[rbox_image]:
            if pre_box['name'] == label:
                if len(pre_box['bbox']) != 0:
                    rboxes[rbox_image].append([pre_box])
                else:
                    pre_box['bbox'] = np.zeros_like(pre_box['bbox'])
                    rboxes[rbox_image].append([pre_box])
    ...
    return rboxes, gboxes #rboxes是预测结果，gboxes是真实结果


def eval(rboxes, gboxes, iou_th, use_07_metric):
    '''计算预测框和真实框之间的iou，以此判断是否检测成功，并统计TP,FP,FN。'''
    rbox_images = rboxes.keys()
    fp = np.zeros(len(rbox_images)) #false positive
    tp = np.zeros(len(rbox_images)) #true positive
    box_num = 0

    for i in range(len(rbox_images)):
        rbox_image = list(rbox_images)[i]
        if len(rboxes[rbox_image][0][0]['bbox']) > 0:
            rbox_lists = np.array(rboxes[rbox_image][0][0]['bbox'])
            if len(gboxes[rbox_image]) > 0:
                ...
                fp[i] += len(rboxes[rbox_image][0][0]['bbox'])
        else:
            continue
    #print('tp:', tp, 'fp:', fp)
    ...
    if box_num == 0:
        for i in range(len(fp)):
            if fp[i] != 0: #有预测框出来，但是真实图上没有这一类，即TP=0, FP>0，FN=0
                prec[i] = 0
            else:
                prec[i] = 1 #没有预测框出来，真实图上也没有这一类，即TP=FP=TN=FN=0
    else:
        fp = np.cumsum(fp)
        tp = np.cumsum(tp)
        #print('tp:', tp, 'fp:', fp)
