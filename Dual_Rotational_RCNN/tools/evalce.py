    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            for i in range(img_num):
                ...
                _fast_rcnn_score_rotate, _detection_category_rotate,_feature_maps_dict\
                    = sess.run([fast_rcnn_decode_boxes_hrotate,img_name_batch,img_batch,gtboxes_and_label,  fast_rcnn_decode_boxes_rotate,
                                fast_rcnn_score_rotate, detection_category_rotate,rpn.feature_maps_dict])
                debug()
                for j in range(3,5,1):
                    ...
                    plt.colorbar()
                    plt.savefig('/files/stu10_file/code/vgg16duibishiyan/tools/xunlian/_{}img_C{}.png'.format(str(_img_name_batch[0]),j))
                end = time.time()
...
if __name__ == '__main__':
    ...
    for label in NAME_LABEL_MAP.keys():
        ...
        rec1, prec1, ap1, box_num1 = eval(rboxes1, gboxes1, 0.1, False, mode=mode)
        #debug()
        #recall = rec[-1]
        recall1 = rec1[-1]
        #precision = prec[-1]
