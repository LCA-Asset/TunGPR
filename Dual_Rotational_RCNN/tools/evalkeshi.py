...
def eval_ship(img_num, mode):
    with tf.Graph().as_default():
        ...
        fast_rcnn_location_rotate_loss, fast_rcnn_classification_rotate_loss,rois,target = fast_rcnn.fast_rcnn_loss()
        ...
        with tf.Session(config=config) as sess:
            ...
            summary_path = os.path.join(FLAGS.summary_path, 'keshi')
            mkdir(summary_path)
            ...
            for i in range(1):
                ...
                _rois,_target,_img_name_batch, _img_batch,_gtboxes_and_label,  _fast_rcnn_decode_boxes_rotate, \
                _fast_rcnn_score_rotate, _detection_category_rotate \
                    = sess.run([rois,target,img_name_batch,img_batch,gtboxes_and_label,  fast_rcnn_decode_boxes_rotate,
                                fast_rcnn_score_rotate, detection_category_rotate])
                
                for j in range(_rois.shape[0]):
                    #plt.figure()
                    #plt.imshow(np.squeeze(_rois[j, :, :, :]))
                    #plt.colorbar()
                    #plt.savefig('/files/stu10_file/code/vgg16duibishiyan/tools/xunlian/_roi{}.png'.format(str(j)))
                    #plt.figure()
                    #plt.imshow(np.squeeze(_target[j, :, :, ::-1]))
                    #plt.colorbar()
                    #plt.savefig('/files/stu10_file/code/vgg16duibishiyan/tools/xunlian/_target{}.png'.format(str(j)))
                    cv2.imwrite('/files/stu10_file/code/vgg16duibishiyan/tools/xunlian/_roi{}.png'.format(str(j)), np.squeeze(_rois[j, :, :, :]))
                    cv2.imwrite('/files/stu10_file/code/vgg16duibishiyan/tools/xunlian/_target{}.png'.format(str(j)), np.squeeze(_target[j, :, :,::-1]))
