...
def train():
    with tf.Graph().as_default():
        ...
        rpn_proposals_boxes, rpn_proposals_scores, rpn_decode_boxes, rpn_decode_boxes, valid_indices, valid_boxes, valid_scores = rpn.rpn_proposals()  # rpn_score shape: [300, ]
        ...
        with tf.name_scope('draw_proposals'):
            ...
            tf.summary.image('rpn/rpn_object_proposals', rpn_proposals_objcet_boxes_in_img)
            tf.summary.image('rpn/rpn_boxes_proposals', rpn_proposals_boxes_in_img)
        # ***********************************************************************************************
        # *                                         Fast RCNN                                           *
        # ***********************************************************************************************
        ...
        lr = tf.train.piecewise_constant(global_step,
                                         boundaries=[np.int64(2000), np.int64(6000), np.int64(8000)],
        ...
        with tf.Session(config=config) as sess:
            ...
            for step in range(cfgs.MAX_ITERATION):
                ...
                if (step > 0 and step % 500 == 0) or (step == cfgs.MAX_ITERATION - 1):
                    ...
                    img_num = 94
                    eva.eval_ship(img_num)
                    fr1 = open('gtboxes_horizontal_dict.pkl', 'rb')  # eval_ship的结果先写入了这个文件，这里又加载进来
                    fr2 = open('predict_horizontal_dict.pkl', 'rb')
                    gtboxes_horizontal_dict = pickle.load(fr1)
                    predict_horizontal_dict = pickle.load(fr2)
                    # predict_horizontal_dict = gtboxes_horizontal_dict

                    R, P, AP, F, num = [], [], [], [], []

                    for label in NAME_LABEL_MAP.keys():  # 统计每一类的结果，然后算所有类的平均
                        if label == 'back_ground':
                            continue

                        rboxes, gboxes = eva.get_single_label_dict(predict_horizontal_dict, gtboxes_horizontal_dict, label)
                        # 获取label类的预测框和真实框列表字典

                        rec, prec, ap, box_num = eva.eval(rboxes, gboxes, 0.5, False)
                        # 计算预测和真实之间的iou并统计TP,FP,FN
                    ...
                    # tf.summary.scalar('validate/Recall', tf.convert_to_tensor(Recall))
                    # tf.summary.scalar('validate/Precision', tf.convert_to_tensor(Precision))
                    # tf.summary.scalar('validate/F_measure', tf.convert_to_tensor(F_measure))
                    # tf.summary.scalar('validate/mAP', tf.convert_to_tensor(mAP))
                    tf.summary.text('validate/Recall', tf.convert_to_tensor(str(Recall)))
                    tf.summary.text('validate/Precision', tf.convert_to_tensor(str(Precision)))
                    tf.summary.text('validate/F_measure', tf.convert_to_tensor(str(F_measure)))
                    tf.summary.text('validate/mAP', tf.convert_to_tensor(str(mAP)))
