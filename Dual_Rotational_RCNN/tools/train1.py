...
img_num = cfgs.IMG_NUM
img_num1 = cfgs.IMG_NUM1
RESTORE_FROM_RPN = False
FLAGS = get_flags_byname(cfgs.NET_NAME)
os.environ["CUDA_VISIBLE_DEVICES"] = cfgs.GPU_GROUP
save_path = os.path.join(FLAGS.summary_path, cfgs.VERSION)
writer = SummaryWriter(save_path)
#save_path1 = os.path.join(FLAGS.summary_path, cfgs.VERSION3)
#writer1 = SummaryWriter(save_path1)
...
def valid(img_num):
    with tf.Graph().as_default():
        ...
        fast_rcnn = build_fast_rcnn1.FastRCNN(img_batch,feature_pyramid=logits,
                                              ...
                                              #fast_rcnn_nms_iou_threshold = cfgs.FAST_RCNN_NMS_IOU_RO_THRESHOLD,
...
# train
        ...
        total_loss = fast_rcnn_total_loss+rpn_total_loss
        ...
        with tf.Session(config=config) as sess:
            ...
            for i in range(img_num):
               _total_loss,_rpn_total_loss,_fast_rcnn_total_loss, _img_name_batch= sess.run([total_loss,rpn_total_loss,fast_rcnn_total_loss,img_name_batch])
               all_loss += _total_loss
               fast_rcnn +=_fast_rcnn_total_loss
               rpn += _rpn_total_loss
               print(""" image_name:{} |\t total_loss_valid:{}|\t
               fast_rcnn_total_loss_valid:{} |\t rpn_total_loss_valid:{} |\t """.format(str(_img_name_batch[0]),_total_loss,_fast_rcnn_total_loss,_rpn_total_loss))
            loss_avg = all_loss/img_num
            fast_rcnn_avg  = fast_rcnn/img_num
            rpn_avg = rpn/img_num
    return loss_avg,fast_rcnn_avg,rpn_avg
...
#                print(""" image_name:{} |\t total_loss_valid:{}|\t
#                fast_rcnn_total_loss_valid:{} |\t rpn_total_loss_valid:{} |\t """.format(str(_img_name_batch[0]),_total_loss,_fast_rcnn_total_loss,_rpn_total_loss))
#             loss_avg = all_loss/img_num1
#             fast_rcnn_avg  = fast_rcnn/img_num1
#             rpn_avg = rpn/img_num1
#     return loss_avg, fast_rcnn_avg,rpn_avg
...
def train():
    with tf.Graph().as_default():
        ...
        # train
        total_loss =slim.losses.get_total_loss()
        #total_loss = fast_rcnn_total_loss+2*rpn_total_loss
        #global_step = tf.train.get_or_create_global_step()
        global_step = slim.get_or_create_global_step()
        #global_step = slim.create_global_step()
        lr = tf.train.piecewise_constant(global_step,
                                         boundaries=[np.int64(9500), np.int64(10500), np.int64(14500)],
...
#                    loss_avg,fast_rcnn_avg,rpn_avg = valid(img_num)
#                    print(""" total_loss_avg:{}|\t fast_rcnn_total_loss_avg:{} |\t
#                    rpn_total_loss_avg:{} |\t""".format(loss_avg,fast_rcnn_avg,rpn_avg))
#                    writer.add_scalar('total_loss_valid', loss_avg, _global_step)
#                    writer.add_scalar('fast_rcnn_total_loss_valid', fast_rcnn_avg, _global_step)
#                    writer.add_scalar('rpn_total_loss_valid', rpn_avg, _global_step)
#                    if min_loss>loss_avg:
#                        min_loss = loss_avg
#                        save_dir = os.path.join(FLAGS.trained_checkpoint, cfgs.VERSION1)
#                        mkdir(save_dir)
#                        save_ckpt = os.path.join(save_dir, 'voc_'+str(_global_step)+'model.ckpt')
#                        saver.save(sess, save_ckpt)
#                        print(' weights had been saved')
#                    
#                    loss_avg1,fast_rcnn_avg1,rpn_avg1 = test(img_num1)
#                    print(""" total_loss_avg:{}|\t fast_rcnn_total_loss_avg:{} |\t
#                    rpn_total_loss_avg:{} |\t""".format(loss_avg1,fast_rcnn_avg1,rpn_avg1))
#                    writer1.add_scalar('total_loss_valid', loss_avg1, _global_step)
#                    writer1.add_scalar('fast_rcnn_total_loss_valid', fast_rcnn_avg1, _global_step)
#                    writer1.add_scalar('rpn_total_loss_valid', rpn_avg1, _global_step)
#                    if min_loss1>loss_avg1:
#                        min_loss1 = loss_avg1
#                        save_dir = os.path.join(FLAGS.trained_checkpoint, cfgs.VERSION3)
#                        mkdir(save_dir)
#                        save_ckpt = os.path.join(save_dir, 'voc_'+str(_global_step)+'model.ckpt')
#                        saver.save(sess, save_ckpt)
#                        print(' weights had been saved')
